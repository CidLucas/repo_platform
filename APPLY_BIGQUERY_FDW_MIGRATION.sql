...
-- =====================================================================
-- This enables BigQuery Foreign Data Wrapper (FDW) in Supabase
-- Eliminates need for heavy google-cloud-bigquery Python SDK
-- =====================================================================
...
-- Try to create vault extension, ignore if already exists or not available
do $$
begin
  create extension if not exists vault with schema vault;
exception
  when others then
    raise notice 'Vault extension not available or already installed: %', sqlerrm;
end
$$;

-- 2. Create BigQuery Foreign Data Wrapper
-- Note: CREATE FOREIGN DATA WRAPPER doesn't support IF NOT EXISTS
...
exception
  when duplicate_object then
    null; -- Wrapper already exists, ignore
end
$$;
...
begin
  ...
  -- Generate server name (sanitize client_id for server name)
  v_server_name := 'bigquery_' || replace(p_client_id, '-', '_');

  -- Check if server already exists
  if exists (select 1 from public.bigquery_servers where client_id = p_client_id) then
    raise exception 'BigQuery server already exists for client: %', p_client_id;
  end if;

  -- Create foreign server using service account key directly
  -- BigQuery FDW accepts sa_key directly as a JSON string
  execute format(
  ...
  -- Store metadata (credentials stored in server options)
  insert into public.bigquery_servers (
    ...
    p_project_id,
    p_dataset_id,
    gen_random_uuid(), -- Placeholder UUID since we're not using vault
    p_location
  );
...
begin
  ...
  -- Generate foreign table name (sanitize client_id for table name)
  v_foreign_table_name := v_schema_name || '.' || replace(p_client_id, '-', '_') || '_' || p_table_name;
  ...
  return jsonb_build_object(
    ...
    'bigquery_table', p_bigquery_table,
    'columns', p_columns
  );
exception
  when others then
...
begin
  ...
  return jsonb_build_object(
    'success', true,
    'server_name', v_server_name,
    'message', 'Server and all foreign tables dropped'
  );
exception
  when others then
    return jsonb_build_object(
...
-- MIGRATION COMPLETE!
-- =====================================================================
-- Next: Test with SELECT create_bigquery_server('test_client', ...);
-- =====================================================================
