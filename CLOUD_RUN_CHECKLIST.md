...
- [ ] **View images in Artifact Registry**
  ...
  gcloud artifacts docker images list \
    us-east1-docker.pkg.dev/$(gcloud config get-value project)/vizu-mono/
  ```
  
  Expected images: 7 services with format `vizu-{service-name}:{timestamp}-{commit-sha}`- [ ] **Test public service (atendente-core)**
  ```bash
  # Get service URL
