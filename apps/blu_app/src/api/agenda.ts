 ...
 * Resolves client_id via RPC so this can be called from the SIGNED_IN
 * auth event before React state is ready.
 */
...
export async function fetchTodaySchedule(clientId: string): Promise<CalendarEvent[]> {
  ...
  const [approvalsResult, calendarResult] = await Promise.allSettled([
    // Approvals with scheduled_for today
    supabase
    ...
    // Google Calendar events via edge function (range=1 day)
