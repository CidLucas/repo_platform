...
/**
 * Right drawer panel.
 * - Desktop (lg+): Persistent sidebar on the right, collapsible
...
export function RightDrawer({
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          'flex-col bg-gray-900 border-l border-border',
...
/** Pill button that opens the mobile right drawer */
export function RightDrawerPill({
  label = 'Histórico',
  icon,
  onClick,
  count,
  ...
  return (
    ...
    >
      ...
      <span>{label}</span>
      {count != null && count > 0 && (
        <span className="text-caption-sm text-blu-400 font-medium ml-1">{count}</span>
      )}
    </button>
  )
}
