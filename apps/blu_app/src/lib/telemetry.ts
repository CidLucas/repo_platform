...
const getDistinctId = (): string | undefined => {
  ...
  return window.localStorage.getItem('blu.telemetry.distinct_id') ?? undefined
}

const setDistinctId = (id: string): void => {
  if (typeof window === 'undefined') return
  window.localStorage.setItem('blu.telemetry.distinct_id', id)
}
...
export const identifyTelemetryUser = (
  distinctId: string,
