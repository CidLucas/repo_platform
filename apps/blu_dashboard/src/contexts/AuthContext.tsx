...
export interface AuthContextType {
  ...
  isLoading: boolean;
  clientId: string | null; // Real client_id from clientes_blu table
  tier: string | null; // Tier from clientes_blu table (FREE, BASIC, SME, PREMIUM, ENTERPRISE, ADMIN)
  signInWithEmail: (
    email: string,
    password: string
...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const initializeClientId = async () => {
    ...
    try {
      ...
      // Fetch tier from clientes_blu table (direct Supabase query, RLS-protected)
      try {
  ...
  useEffect(() => {
    ...
    const {
      ...
      if (_event === 'SIGNED_OUT') {
        ...
        localStorage.removeItem('blu_client_tier');
        resetTelemetry();
      }
    });
