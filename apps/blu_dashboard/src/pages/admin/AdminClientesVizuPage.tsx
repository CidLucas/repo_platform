...
const emptyClientForm: ClienteBluCreate = {
  nome_empresa: '',
  tier: 'BASIC',
  enabled_tools: [],
  collection_rag: '',
...
function AdminClientesBluPage() {
  ...
  // State
  const [clients, setClients] = useState<ClienteBlu[]>([]);
  const [allTools, setAllTools] = useState<ToolMetadata[]>([]);
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [editingClient, setEditingClient] = useState<ClienteBlu | null>(null);
  const [deletingClient, setDeletingClient] = useState<ClienteBlu | null>(null);
  const [formData, setFormData] = useState<ClienteBluCreate>(emptyClientForm);
  ...
  const handleEditClient = (client: ClienteBlu) => {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingClient) {
        // Update existing
        const updateData: ClienteBluUpdate = {
          ...formData,
        };
  ...
  // Confirm delete
  const handleConfirmDelete = (client: ClienteBlu) => {
...
export default AdminClientesBluPage;
