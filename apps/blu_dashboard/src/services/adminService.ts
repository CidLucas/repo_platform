...
export interface ClienteBluCreate {
  nome_empresa: string;
  tier?: string;
...
export interface ClienteBluUpdate {
  nome_empresa?: string;
  tier?: string;
  enabled_tools?: string[];
  collection_rag?: string;
 ...
 */
export async function getClient(clientId: string): Promise<ClienteBlu> {
  const response = await fetch(
  ...
  return handleResponse<ClienteBlu>(response);
}

/**
 * Create a new client
 */
export async function createClient(data: ClienteBluCreate): Promise<ClienteBlu> {
  const response = await fetch(
