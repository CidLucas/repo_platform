...
export function useAgentBuilder() {
  ...
  const addContextField = useCallback((field: ContextFieldDefinition) => {
    setState((prev) => ({
      ...prev,
      formData: { ...prev.formData, required_context: [...prev.formData.required_context, field] },
      isDirty: true,
    }))
  }, [])
  ...
  const updateContextField = useCallback((index: number, field: ContextFieldDefinition) => {
    setState((prev) => {
      ...
      updated[index] = field
      return { ...prev, formData: { ...prev.formData, required_context: updated }, isDirty: true }
    })
  }, [])
  ...
  const save = useCallback(async (): Promise<CatalogAgent | null> => {
    ...
    try {
      ...
      if (editingAgentId) {
        saved = await updateCatalogAgent(editingAgentId, formData as CatalogAgentUpdate, accessToken)
      } else {
        saved = await createCatalogAgent(formData as CatalogAgentCreate, accessToken)
      }
