 ...
 * Ported from apps/blu_app/src/services/standaloneAgentService.ts
 */
...
export interface UploadedFile {
  ...
  file_name: string
  storage_path: string
  columns_schema?: Array<{ name: string; type: string; sample: unknown[] }>
...
export interface RequirementsStatus {
  ...
  missing: Array<{ field_name: string; label: string; type: string }>
  files_required: {
...
export async function fetchAgentCatalog(accessToken: string, _tier?: string): Promise<AgentCatalogEntry[]> {
  const response = await fetch(`${AGENT_API_URL}/v1/catalog/agents`, {
    headers: { Authorization: `Bearer ${accessToken}`, 'Content-Type': 'application/json' },
  })
  if (!response.ok) throw new Error(`Failed to fetch agent catalog: ${response.status}`)
  return response.json()
}

export async function fetchAgentDetails(agentId: string, accessToken: string): Promise<AgentCatalogEntry> {
...
export async function createSession(agentCatalogId: string, accessToken: string): Promise<StandaloneAgentSession> {
...
export async function saveConfigField(
  ...
  const response = await fetch(
    `${AGENT_API_URL}/v1/sessions/${sessionId}/config/${fieldName}`,
    {
      method: 'POST',
      headers: { Authorization: `Bearer ${accessToken}`, 'Content-Type': 'application/json' },
  ...
  if (!response.ok) throw new Error(`Failed to save config field: ${response.status}`)
}

export async function finalizeConfig(sessionId: string, accessToken: string): Promise<StandaloneAgentSession> {
  const response = await fetch(`${AGENT_API_URL}/v1/sessions/${sessionId}/config/finalize`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${accessToken}`, 'Content-Type': 'application/json' },
  })
  if (!response.ok) throw new Error(`Failed to finalize config: ${response.status}`)
  return response.json()
}

// ── File Upload API ────────────────────────────────────────────

export async function uploadCsvFile(sessionId: string, file: File, accessToken: string): Promise<UploadedFile> {
  ...
  if (!response.ok) throw new Error(`Failed to upload CSV: ${response.status}`)
  return response.json()
}
...
export async function linkDocumentToSession(
  ...
  if (!response.ok) throw new Error(`Failed to link document to session: ${response.status}`)
}
...
export async function peekCsvColumns(
  ...
  sessionId: string,
  accessToken: string,
): Promise<{ columns: Array<{ name: string; type: string }>; sample_rows: Array<Record<string, unknown>> }> {
  const response = await fetch(
    `${AGENT_API_URL}/v1/sessions/${sessionId}/csv/${fileId}/peek`,
    { headers: { Authorization: `Bearer ${accessToken}`, 'Content-Type': 'application/json' } },
  )
  if (!response.ok) throw new Error(`Failed to peek CSV: ${response.status}`)
  return response.json()
}

// ── Streaming API ──────────────────────────────────────────────

async function* streamSse(url: string, message: string, accessToken: string): AsyncGenerator<StreamEvent> {
...
export async function* streamConfigHelperChat(
  ...
  yield* streamSse(`${AGENT_API_URL}/v1/sessions/${sessionId}/chat/config`, message, accessToken)
}
...
export async function* streamAgentChat(
  ...
  yield* streamSse(`${AGENT_API_URL}/v1/sessions/${sessionId}/chat/agent`, message, accessToken)
}

// ── Google OAuth ───────────────────────────────────────────────

export async function initiateGoogleAuth(accessToken: string): Promise<{ auth_url: string; state: string }> {
  const response = await fetch(`${TOOL_POOL_API_URL}/integrations/google/auth/initiate`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${accessToken}`, 'Content-Type': 'application/json' },
  })
  if (!response.ok) throw new Error(`Failed to initiate Google auth: ${response.status}`)
  return response.json()
}
...
export async function fetchGoogleAccounts(
  ...
  if (!response.ok) throw new Error(`Failed to fetch Google accounts: ${response.status}`)
  return response.json()
}
...
export async function linkGoogleToSession(
  ...
  if (!response.ok) throw new Error(`Failed to link Google account: ${response.status}`)
}

// ── Session Activation ─────────────────────────────────────────

export async function activateSession(sessionId: string, accessToken: string): Promise<StandaloneAgentSession> {
  const response = await fetch(`${AGENT_API_URL}/v1/sessions/${sessionId}/activate`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${accessToken}`, 'Content-Type': 'application/json' },
  })
  if (!response.ok) throw new Error(`Failed to activate session: ${response.status}`)
  return response.json()
}

export async function deleteFile(sessionId: string, fileId: string, accessToken: string): Promise<void> {
