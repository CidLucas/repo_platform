"""
FastMCP middleware helpers for blu_auth.

Provides a decorator factory `mcp_inject_client_id` which, when applied to
tool callables, will attempt to resolve `client_id` from the FastMCP
`AccessToken` (via `get_access_token`) and inject it into the tool kwargs
as `client_id` when not already provided.

This middleware is intentionally lightweight and non-invasive: if no
access token is present or no mapping exists, it leaves the kwargs
unchanged and lets the tool handle auth/fallbacks.
"""

import inspect
import logging
from collections.abc import Awaitable, Callable
from functools import wraps

logger = logging.getLogger(__name__)

from fastmcp.exceptions import ToolError
from fastmcp.server.dependencies import get_access_token, get_http_headers

from blu_auth.adapters.context_service_adapter import external_user_lookup_from_context_service


def mcp_inject_client_id(get_context_service_fn: Callable[[], object]) -> Callable[[Callable[..., Awaitable]], Callable[..., Awaitable]]:
    """
    Factory that returns a decorator for FastMCP tool callables.

    Args:
        get_context_service_fn: Callable (no-arg) that returns a ContextService
            instance at runtime (typically imported from the host app's
            dependencies module, e.g. `tool_pool_api.server.dependencies.get_context_service`).

    Usage:
        @mcp_inject_client_id(get_context_service)
        async def my_tool(..., client_id: str | None = None):
            ...
    """

    def decorator(fn: Callable[..., Awaitable]) -> Callable[..., Awaitable]:
        sig = inspect.signature(fn)

        @wraps(fn)
        async def wrapper(*args, **kwargs):
            # Priority for client_id resolution:
            # 1. X-Cliente-Id header (set by atendente_core after JWT validation)
            # 2. AccessToken claims.sub -> lookup via ContextService
            # 3. Caller-provided client_id (fallback)

            client_id = None

            # Method 1: Check X-Cliente-Id header (preferred for internal calls)
            # This is set by atendente_core which already validated the JWT
            headers = get_http_headers(include_all=True)
            header_client_id = headers.get("x-cliente-id")
            if header_client_id:
                client_id = header_client_id
                logger.info(f"[mcp_inject_client_id] Got client_id from X-Cliente-Id header: {client_id}")

            # Method 2: Try JWT claims.sub if no header
            if not client_id:
                try:
                    access_token = get_access_token()
                except Exception:
                    access_token = None

                if access_token:
                    external_user_id = getattr(access_token, "claims", {}).get("sub") if hasattr(access_token, "claims") else None
                    if external_user_id:
                        logger.debug(f"[mcp_inject_client_id] Trying to resolve from JWT sub: {external_user_id}")
                        try:
                            ctx_service = get_context_service_fn()
                            lookup = external_user_lookup_from_context_service(ctx_service)
                            client_id = await lookup(external_user_id)
                            if client_id:
                                logger.info(f"[mcp_inject_client_id] Resolved client_id from JWT: {client_id}")
                        except ToolError:
                            raise
                        except Exception as e:
                            logger.warning(f"[mcp_inject_client_id] Failed to resolve from JWT: {e}")

            # Inject client_id if resolved
            if client_id:
                kwargs["client_id"] = str(client_id)
            else:
                logger.warning("[mcp_inject_client_id] No client_id resolved, using caller-provided value")

            # Also inject session_id from X-Session-Id header if present
            # Only inject if the function actually accepts session_id
            header_session_id = headers.get("x-session-id")
            if header_session_id and "session_id" in sig.parameters:
                kwargs["session_id"] = str(header_session_id)
                logger.debug(f"[mcp_inject_client_id] Got session_id from X-Session-Id header: {header_session_id}")

            return await fn(*args, **kwargs)

        return wrapper

    return decorator
