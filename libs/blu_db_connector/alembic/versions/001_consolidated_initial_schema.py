"""Consolidated initial schema from blu_models

Revision ID: 001_consolidated_initial_schema
Revises:
Create Date: 2025-12-08 12:00:00.000000

This migration creates the complete database schema from blu_models in a single,
...
def upgrade() -> None:
    """Create all tables from blu_models"""

    # Create ENUM types - must match blu_models definitions
    op.execute("""
    ...
    # cliente_blu - main customer entity
    op.create_table(
        'cliente_blu',
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(['client_id'], ['cliente_blu.id'], ondelete='CASCADE'),
    )
    ...
    op.create_index('ix_fonte_de_dados_cliente_blu', 'fonte_de_dados', ['client_id'])
    op.create_index('ix_credencial_servico_cliente_blu', 'credencial_servico_externo', ['client_id'])
    op.create_index('ix_cliente_final_cliente_blu', 'cliente_final', ['client_id'])
    op.create_index('ix_conversa_cliente_blu', 'conversa', ['client_id'])
    ...
    op.create_index('ix_hitl_review_cliente_blu', 'hitl_review', ['client_id'])
    op.create_index('ix_experiment_cliente_blu', 'experiment', ['client_id'])
    op.create_index('ix_integration_connection_cliente_blu', 'integration_connection', ['client_id'])
def downgrade() -> None:
    ...
    op.drop_table('cliente_blu', if_exists=True)
