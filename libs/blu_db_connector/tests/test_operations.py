...
from blu_models import ClienteBlu, FonteDeDados, TipoFonte
from blu_models.cliente_blu import TierCliente, TipoCliente


def test_create_cliente_blu(db_session: Session):
    """
    Testa a criação de uma nova entrada na tabela ClienteBlu.
    ...
    cliente_do_banco = (
        db_session.query(ClienteBlu).filter_by(nome_empresa="Cliente Teste SA").one()
    )
    ...
    def test_create_fonte_de_dados_for_cliente(db_session: Session):
        """
        Testa a criação de uma FonteDeDados e seu relacionamento com ClienteBlu.
        """

    # 1. Setup: Criar um cliente "pai" primeiro
    cliente_pai = ClienteBlu(
    ...
    assert cliente_pai.fontes_de_dados[0].caminho == "https://blu.ai/docs"
