...
Usage:
    ...
    python -m blu_experiment_service.cli run <manifest.yaml>

    # Run with legacy runner (httpx only)
    python -m blu_experiment_service.cli run <manifest.yaml> --legacy

    # Sync manifest to Langfuse Dataset
    python -m blu_experiment_service.cli sync <manifest.yaml>

    # Classify results and route to HITL
    python -m blu_experiment_service.cli classify <run_id>

    # Export approved cases to training dataset
    python -m blu_experiment_service.cli export <run_id> --format jsonl
"""
