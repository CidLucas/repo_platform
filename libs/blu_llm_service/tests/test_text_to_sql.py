...
blu_prompt_management infrastructure (PromptLoader, VariableExtractor,
...
from blu_llm_service.text_to_sql import (
    TextToSqlPrompt,
...
class TestTextToSqlPrompt:
    ...
    async def test_build_from_context(self, mock_prompt_loader, mock_context):
        """Test build_from_context() with BluClientContext."""
        prompt_obj = TextToSqlPrompt(prompt_loader=mock_prompt_loader)
...
class TestTextToSqlPromptIntegration:
    """Integration tests with blu_prompt_management."""
    ...
    async def test_uses_prompt_loader(self):
        ...
        with patch("blu_prompt_management.loader.PromptLoader.load") as mock_load:
            mock_template = Mock()
