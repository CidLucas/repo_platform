...
```bash
poetry add blu-parsers --path ../libs/blu_parsers
```
...
```python
from blu_parsers import PDFParser, CSVParser, TXTParser, get_parser_for_file
...
```python
from blu_parsers import TextChunker, ChunkingStrategy
...
```python
from blu_parsers import parse_and_chunk
