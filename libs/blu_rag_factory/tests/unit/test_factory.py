...
@pytest.mark.asyncio
async def test_rag_factory_success(mocker: MagicMock, mock_blu_client_context: BluClientContext):
    """
    ...
    assert "executar_rag_cliente" in mock_blu_client_context.enabled_tools
...
@pytest.mark.asyncio
async def test_rag_factory_disabled(mocker: MagicMock, mock_blu_client_context: BluClientContext):
    """Testa se a factory retorna None se a ferramenta está desabilitada."""
    # 1. Arrange
    mock_llm = mocker.MagicMock(spec=BaseChatModel)

    # DESABILITA a ferramenta removendo do enabled_tools
    mock_blu_client_context.enabled_tools = []
