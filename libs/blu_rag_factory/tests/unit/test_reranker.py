...
from blu_rag_factory.reranker import (
    CohereReranker,
...
class TestCohereReranker:
    ...
    async def test_arerank_sorts_by_score(self, sample_documents):
        ...
        with patch("blu_rag_factory.reranker.httpx.AsyncClient") as mock_client_cls:
...
class TestCrossEncoderReranker:
    ...
    @patch("blu_rag_factory.reranker._get_cross_encoder")
...
class TestFactoryRerankerSelection:
    ...
    def _mock_build_prompt(self, mocker):
        ...
        mocker.patch(
            "blu_rag_factory.factory.build_prompt",
            new_callable=AsyncMock,
            return_value="You are an assistant.\n\nCONTEXT:\n{context}\n\nQUESTION:\n{question}\n\nRESPONSE:",
        )

    @pytest.mark.asyncio
    async def test_factory_creates_cross_encoder_reranker(self, mocker, mock_blu_client_context):
        """When reranker_type='cross-encoder', factory should use CrossEncoderReranker."""
        from blu_rag_factory.factory import create_rag_runnable
        ...
        mock_blu_client_context.available_tools = {
            "rag_search_config": {
                "rerank": True,
        ...
        runnable = await create_rag_runnable(mock_blu_client_context, mock_llm)
        assert runnable is not None

    @pytest.mark.asyncio
    async def test_factory_creates_cohere_reranker(self, mocker, mock_blu_client_context):
    ...
    @pytest.mark.asyncio
    async def test_factory_creates_llm_reranker(self, mocker, mock_blu_client_context):
    ...
    @pytest.mark.asyncio
    async def test_factory_defaults_to_cohere(self, mocker, mock_blu_client_context):
    ...
    @pytest.mark.asyncio
    async def test_factory_no_reranker_when_disabled(self, mocker, mock_blu_client_context):
