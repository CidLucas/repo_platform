...
def test_retriever_returns_documents(retriever: SupabaseVectorRetriever):
    ...
    with patch("blu_rag_factory.retriever.httpx.post", return_value=mock_response) as mock_post:
...
def test_retriever_empty_results(retriever: SupabaseVectorRetriever):
    ...
    with patch("blu_rag_factory.retriever.httpx.post", return_value=mock_response):
...
async def test_hybrid_async_retrieval(hybrid_retriever: HybridRetriever):
    ...
    with patch("blu_rag_factory.retriever.httpx.AsyncClient", return_value=mock_client):
