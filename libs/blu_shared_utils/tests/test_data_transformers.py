...
from blu_shared_utils.data_transformers import transform_data
...
def test_transform_unix_timestamp_seconds_to_datetime():
    ...
    mappings = {
        BluCanonicalColumn.INVOICE_DATE: ColumnConfig(
            blu_name=BluCanonicalColumn.INVOICE_DATE,
    ...
    assert BluCanonicalColumn.INVOICE_DATE.value in df_transformed.columns
    assert 'created_at' not in df_transformed.columns

    # 2. O formato deve ser convertido para datetime
    assert pd.api.types.is_datetime64_any_dtype(df_transformed[BluCanonicalColumn.INVOICE_DATE.value])

    # 3. Os valores devem ser corretos (1/Jan/2021 e 1/Jan/2022)
    expected_dates = [datetime(2021, 1, 1, 0, 0), datetime(2022, 1, 1, 0, 0)]
    assert all(df_transformed[BluCanonicalColumn.INVOICE_DATE.value] == expected_dates)
...
def test_transform_data_applies_text_normalization():
    ...
    # SETUP: Configuração
    mappings = {
        BluCanonicalColumn.PRODUCT_NAME: ColumnConfig(
            blu_name=BluCanonicalColumn.PRODUCT_NAME,
    ...
    column_name = BluCanonicalColumn.PRODUCT_NAME.value
