...
- as `ExecutionConfig.allowed_views` / `allowed_columns` by
  `blu_sql_factory.TextToSqlExecutor` in any new code path that needs an
...
#: Mandatory tenancy filter — `blu_sql_factory.SqlRewriter` enforces this
...
#: `blu_sql_factory.config.allowlist.json` for legacy code paths.
...
class AnalyticsV2Contract:
    ...
    def to_execution_config_kwargs(self, *, client_id: str) -> dict:
        """Render kwargs for `blu_sql_factory.ExecutionConfig`."""
        return {
            "client_id": client_id,
