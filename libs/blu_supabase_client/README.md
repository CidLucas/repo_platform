...
# Consultas
response = client.table("cliente_blu").select("*").eq("id", "uuid-here").execute()
clientes = response.data

# Inserção
response = client.table("cliente_blu").insert({"nome_empresa": "Teste"}).execute()

# Update
response = client.table("cliente_blu").update({"nome_empresa": "Novo Nome"}).eq("id", "uuid").execute()

# Delete
response = client.table("cliente_blu").delete().eq("id", "uuid").execute()
```
