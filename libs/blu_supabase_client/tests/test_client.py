...
class TestSupabaseConfig:
    ...
    def test_config_from_env(self):
        ...
        from blu_supabase_client.client import SupabaseConfig
...
class TestSupabaseCRUD:
    ...
    def test_get_cliente_blu_by_api_key_found(self, crud, mock_client):
        ...
        result = crud.get_cliente_blu_by_api_key("test-key")

        assert result is not None
        assert result["id"] == "test-uuid"
        assert result["nome_empresa"] == "Test Corp"
        mock_client.table.assert_called_with("clientes_blu")

    def test_get_cliente_blu_by_api_key_not_found(self, crud, mock_client):
        ...
        result = crud.get_cliente_blu_by_api_key("nonexistent-key")

        assert result is None

    def test_get_cliente_blu_by_id(self, crud, mock_client):
        ...
        result = crud.get_cliente_blu_by_id(UUID("12345678-1234-1234-1234-123456789abc"))

        assert result is not None
        assert result["nome_empresa"] == "Test Corp"

    def test_create_cliente_blu(self, crud, mock_client):
        ...
        result = crud.create_cliente_blu({"nome_empresa": "New Corp", "tipo_cliente": "AGENDAMENTO"})

        assert result is not None
        assert result["nome_empresa"] == "New Corp"

    def test_update_cliente_blu(self, crud, mock_client):
        ...
        result = crud.update_cliente_blu(
    ...
    def test_delete_cliente_blu(self, crud, mock_client):
        """Test deleting a cliente."""
        from uuid import UUID

        mock_client.table.return_value.delete.return_value.eq.return_value.execute.return_value = MagicMock()

        result = crud.delete_cliente_blu(UUID("12345678-1234-1234-1234-123456789abc"))
...
class TestRLSContext:
    ...
    def test_set_rls_context(self):
        ...
        from blu_supabase_client.client import set_rls_context
