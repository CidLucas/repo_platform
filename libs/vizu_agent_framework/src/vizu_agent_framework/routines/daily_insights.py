...
Pipeline (per tenant):
    ...
    4. If the tenant has an enabled `client_routines` row for routine_id
       ...
       send a WhatsApp digest via `vizu_twilio_client`.
    5. Append a single `audit_log` entry summarising the run.

Trigger paths
─────────────
- CLI: `python -m vizu_agent_framework.routines.daily_insights --client-id <uuid>`
...
- We do NOT spin up the full LangGraph supervisor for this routine — the work
  ...
  `vizu_agent_framework.AgentBuilder` would require an MCP server, Redis
