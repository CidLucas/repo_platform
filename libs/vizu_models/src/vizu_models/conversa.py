...
class MensagemBase(SQLModel):
    ...
    remetente: Remetente = Field(
        sa_column=Column(
            SAEnum(Remetente, values_callable=lambda x: [e.value for e in x], name="remetente_enum", create_type=False)
        )
    )
...
class Mensagem(MensagemBase, table=True):
    ...
    conversa_id: uuid.UUID = Field(sa_column=Column(pgUUID(as_uuid=True), ForeignKey("conversa.id")))
