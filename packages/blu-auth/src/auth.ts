 ...
 */
export async function resolveClientId(providedClientId?: string): Promise<string> {
  ...
  if (fromApp) return fromApp as string
  ...
  if (lookupError) {
    throw new Error(`Failed to resolve client by external_user_id: ${lookupError.message}`)
  }

  if (data?.client_id) return data.client_id as string

  throw new Error(`Client not found for user: ${user.id}`)
}
