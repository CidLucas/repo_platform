...
class TwilioService:
    ...
    def create_conversation(self, group_name: str) -> Optional[str]:
        try:
            conversation = self.client.conversations.v1.conversations.create(friendly_name=group_name)
    ...
    def add_participant_to_conversation(self, conversation_sid: str, participant_number: str) -> Optional[str]:
        try:
            # O friendly_name pode ser adicionado se necessário, mas não é um parâmetro direto do `create`
            participant = self.client.conversations.v1.conversations(conversation_sid).participants.create(
    ...
    def send_system_message(self, conversation_sid: str, message_body: str) -> Optional[str]:
        try:
            message = self.client.conversations.v1.conversations(conversation_sid).messages.create(
                author='system',
                body=message_body
            )
