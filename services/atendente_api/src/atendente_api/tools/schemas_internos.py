...
class SQLQuery(BaseModel):
    ...
    db_credentials: Dict[str, Any] = Field(description="Credenciais para conexão com o banco de dados.")
...
class SchedulingTool(BaseModel):
    ...
    service: Optional[str] = Field(None, description="O serviço que o cliente deseja agendar, ex: 'corte de cabelo'.")
    date: Optional[str] = Field(None, description="A data para o agendamento no formato AAAA-MM-DD.")
    time: Optional[str] = Field(None, description="A hora para o agendamento no formato HH:MM.")
    cancellation_id: Optional[str] = Field(None, description="O ID do agendamento a ser cancelado.")
