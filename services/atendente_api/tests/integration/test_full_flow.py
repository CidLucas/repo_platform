...
from vizu_db_connector.models.cliente_vizu import ClienteVizu, TipoCliente, TierCliente
...
def test_e2e_flow_with_db_and_redis_cache(
    ...
    novo_cliente = ClienteVizu(
        id=cliente_id, api_key=test_api_key, nome_empresa="Padaria Teste",
        tipo_cliente=TipoCliente.EXTERNO, tier=TierCliente.SME
    )
    nova_configuracao = ConfiguracaoNegocio(
        cliente_vizu_id=cliente_id, prompt_base="Seja um assistente de padaria.",
        ferramenta_rag_habilitada=True
    )
    db_session.add(novo_cliente)
    db_session.add(nova_configuracao)
    db_session.commit()

    # --- 2. EXECUÇÃO E VERIFICAÇÃO (PRIMEIRA CHAMADA - CACHE MISS) ---
    # NÃO precisamos mais mockar a query do banco de dados!
    with patch("atendente_api.api.routes.agent_graph") as mock_agent_graph:

        mock_agent_graph.invoke.return_value = {"messages": [MagicMock(content="Olá! Bem-vindo à Padaria Teste.")]}
