...
def execute_scheduling_action(validated_input: SchedulingTool) -> Dict[str, Any]:
    ...
    try:
        ...
        service = GoogleCalendarService(
            refresh_token=validated_input.google_refresh_token
        )

        if intent == "book":
            if not all(
                [validated_input.service, validated_input.date, validated_input.time]
            ):
            ...
            result = service.book_event(
                summary=validated_input.service, start_time=start_dt, end_time=end_dt
            )
            return {
                "result": f"Agendamento confirmado! Link do evento: {result.get('link')}"
            }
