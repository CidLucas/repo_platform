...
from vizu_models import ConfiguracaoNegocioCreate, ConfiguracaoNegocioUpdate, ConfiguracaoNegocio, ClienteVizu
...
def create_configuracao(
    *,
    ...
    cliente = db_session.query(ClienteVizu).filter(ClienteVizu.id == config_in.cliente_vizu_id).first()
    if not cliente:
    ...
    if config_existente:
        ...
        raise HTTPException(status_code=400, detail="Uma configuração já existe para este cliente. Use PUT para atualizar.")

    # Map incoming payload into cliente fields
    cliente.prompt_base = getattr(config_in, "prompt_base", cliente.prompt_base)
    cliente.horario_funcionamento = getattr(config_in, "horario_funcionamento", cliente.horario_funcionamento)
    cliente.ferramenta_rag_habilitada = getattr(config_in, "ferramenta_rag_habilitada", cliente.ferramenta_rag_habilitada)
    cliente.ferramenta_sql_habilitada = getattr(config_in, "ferramenta_sql_habilitada", cliente.ferramenta_sql_habilitada)
    cliente.ferramenta_agendamento_habilitada = getattr(config_in, "ferramenta_agendamento_habilitada", cliente.ferramenta_agendamento_habilitada)
    cliente.collection_rag = getattr(config_in, "collection_rag", cliente.collection_rag)
...
def update_configuracao(
    *,
    ...
    cliente = db_session.query(ClienteVizu).filter(ClienteVizu.id == config_db.cliente_vizu_id).first()
    if not cliente:
