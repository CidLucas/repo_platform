...
class ConfiguracaoService(
    BaseCRUD[
        ConfiguracaoNegocioBase, ConfiguracaoNegocioCreate, ConfiguracaoNegocioUpdate
    ]
):
    def get_by_cliente(
        self, db_session: Session, *, cliente_vizu_id: str
    ) -> ConfiguracaoNegocioBase | None:
        return (
            db_session.query(self.model)
            .filter(self.model.cliente_vizu_id == cliente_vizu_id)
            .first()
        )
