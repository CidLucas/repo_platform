...
from file_upload_api.api.dependencies import get_cliente_vizu_id_from_token # (Dependência de autenticação)
...
def upload_file(
    ...
    try:
        logger.info(f"Recebida requisição de upload de {cliente_vizu_id} para o arquivo: {file.filename}")
        ...
        logger.error(f"Erro ao processar upload para {cliente_vizu_id}: {e}", exc_info=True)
