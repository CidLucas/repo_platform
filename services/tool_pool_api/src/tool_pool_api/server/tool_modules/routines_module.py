...
async def _listar_insights_cliente_logic(
    ctx: Context,
    client_id: str | None = None,
    room: str | None = None,
    severity: str | None = None,
    incluir_dispensados: bool = False,
    limite: int = 20,
) -> dict:
    """
    List the insight cards generated by the client's routines.

    Cards live in public.client_insights (written by storage.save_insights)
    and are shown in each room's dashboard. This tool lets agents read them
    back to answer questions like "o que dizem meus cards de hoje?".
    """
    if not client_id:
        raise ToolError("client_id não encontrado. Certifique-se de estar autenticado.")
    ...
    if severity and severity not in _VALID_SEVERITIES:
        raise ToolError(
            f"severity inválida: {severity}. Use uma de {', '.join(_VALID_SEVERITIES)}."
        )
    ...
    try:
        ...
        query = db.table("client_insights").select(
            "id, room, kpi, title, body, observation, recommendation, severity, "
            "metric_value, baseline_value, variance_pct, run_date, generated_at, dismissed"
        ...
        if not incluir_dispensados:
            query = query.eq("dismissed", False)

        result = query.order("generated_at", desc=True).limit(limite).execute()

        insights = result.data or []
        logger.info(
            f"[Rotinas] Listou {len(insights)} insights para {client_id} "
            f"(room={room}, severity={severity})"
        )
        ...
        logger.exception(f"[Rotinas] Erro ao listar insights: {e}")
        raise ToolError(f"Erro ao listar insights: {e}")
...
def register_tools(mcp: FastMCP) -> list[str]:
    ...
    mcp.tool(
        ...
        description=(
            "List the insight cards generated by the client's routines (shown in each "
            "room's dashboard). Returns title, body, observation, recommendation, "
            "severity, metric values, and dates. Parameters (all optional): "
            "room (financeiro|clientes|compras|agenda|estrategia), "
            "severity (info|warning|error), incluir_dispensados (bool, default false), "
            "limite (int, default 20, max 50). Use to answer questions about the "
            "client's current cards, alerts, and recommendations."
        ),
    )(mcp_inject_client_id(get_context_service)(_listar_insights_cliente_logic))
