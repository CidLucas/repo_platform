...
# 3. Configure o factory para retornar o mock do runnable (como antes)
    ...
    with patch("src.tool_pool_api.server.tools.get_context_service", return_value=mock_ctx_service) as p_ctx_svc, \
         patch("src.tool_pool_api.server.tools.get_access_token", return_value=mock_token) as p_token, \
         patch("src.tool_pool_api.server.tools.load_context_from_token", mock_load_context) as p_load_ctx, \
         patch("src.tool_pool_api.server.tools.create_rag_runnable", mock_create_rag) as p_create_rag:
