...
-- Remove incompatible overloads so PostgREST can resolve the RPC deterministically.
DROP FUNCTION IF EXISTS public.create_bigquery_server(text, jsonb, text, text, text);
DROP FUNCTION IF EXISTS public.create_bigquery_server(uuid, text, text, text, text);

CREATE FUNCTION public.create_bigquery_server(
	p_client_id uuid,
