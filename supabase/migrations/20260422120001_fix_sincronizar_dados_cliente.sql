-- =============================================================================
-- Migration: Fix sincronizar_dados_cliente for pg_cron execution
-- Date: 2026-04-22
-- Purpose:
--   1. Set statement_timeout inside the function (25 min safety net)
--   2. Guard against NULL column_mapping (uses direct SELECT * when no mapping)
--   3. Align connector_sync_history target_table to fato_transacoes (not vendas)
--   4. Add progress_pct updates to reg_jobs from within the sync function
--      so the frontend sees real progress, not stuck at 5%
--   5. Update reg_jobs.progress_pct at key stages so polling is informative
-- =============================================================================
...
BEGIN
  -- Safety net: 25 minutes. The pg_cron worker has no wall-clock limit,
  -- but we still want a hard ceiling so a single bad sync can't block forever.
  SET LOCAL statement_timeout = '1500000'; -- 25 min in ms

  RAISE LOG '[sincronizar_dados_cliente] START client=%, credential=%, force=%',
    p_client_id, p_credential_id, p_force_full_sync;

  -- ── STEP 1: Look up data source (10%) ────────────────────────────────────
  SELECT
    cds.id,
  ...
  RAISE LOG '[sincronizar_dados_cliente] foreign_table=%, target=%, mapping_keys=%',
    v_foreign_table, v_target_table,
    CASE WHEN v_column_mapping IS NOT NULL
         THEN jsonb_array_length(jsonb_object_keys_as_array(v_column_mapping))::text
         ELSE 'NULL (will use SELECT *)'
    END;

  -- ── STEP 3: Watermark — incremental vs full ───────────────────────────────
  SELECT
  ...
  -- ── STEP 4: Find the associated reg_jobs row (created by run-sync EF) ─────
  ...
  -- ── STEP 5: Create connector_sync_history record ──────────────────────────
  ...
  -- ── STEP 6: Progress 20% — clear old data if full sync ───────────────────
  ...
  -- ── STEP 7: Progress 30% — extract from BigQuery FDW ─────────────────────
  IF v_job_id IS NOT NULL THEN
  ...
  v_extract_result := public.extract_bigquery_data(
    ...
    p_column_mapping    := v_column_mapping,  -- NULL is handled by extract_bigquery_data (SELECT *)
  ...
  -- ── STEP 8: Progress 80% — refresh aggregates ────────────────────────────
  IF v_job_id IS NOT NULL THEN
  ...
  EXCEPTION WHEN OTHERS THEN
    -- Non-fatal: aggregate refresh failure doesn't invalidate the data load
...
EXCEPTION WHEN OTHERS THEN
  ...
  -- Best-effort update — v_sync_id may be 0 if the INSERT hadn't happened yet
...
-- Helper used in logging above — only define if not already present
CREATE OR REPLACE FUNCTION public.jsonb_object_keys_as_array(j JSONB)
RETURNS TEXT[]
LANGUAGE sql IMMUTABLE AS $$
  SELECT ARRAY(SELECT jsonb_object_keys(j));
$$;

GRANT EXECUTE ON FUNCTION public.sincronizar_dados_cliente(uuid, integer, boolean) TO authenticated, service_role, postgres;
GRANT EXECUTE ON FUNCTION public.jsonb_object_keys_as_array(jsonb) TO authenticated, service_role, postgres;
