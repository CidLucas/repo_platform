-- =============================================================================
-- Migration: Fix jsonb_object_length compatibility in sincronizar_dados_cliente
-- Date: 2026-04-22
-- Root cause:
--   Some Postgres variants in Supabase don't expose jsonb_object_length(jsonb).
--   Use a portable expression based on jsonb_object_keys instead.
-- =============================================================================
...
BEGIN
  ...
  RAISE LOG '[sincronizar_dados_cliente] foreign_table=%, target=%, mapping_keys=%',
    ...
    CASE
      ...
      WHEN jsonb_typeof(v_column_mapping) = 'object' THEN (
        SELECT count(*)::text FROM jsonb_object_keys(v_column_mapping)
      )
