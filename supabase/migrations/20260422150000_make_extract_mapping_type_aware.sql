-- =============================================================================
-- Migration: Make extract_bigquery_data type-aware for mapped inserts
-- Date: 2026-04-22
-- Problem:
--   Mapped INSERT used raw source expressions, causing runtime errors like:
--   "column ... is of type timestamp with time zone but expression is of type text"
--
-- Fix:
--   - Keep only valid source/target mapping pairs
--   - Cast each source expression to destination column type
--   - Convert empty string to NULL before casting for non-text columns
--   - Add client_id only when destination table has this column
-- =============================================================================
  ...
  ELSE
    ...
    LOOP
      ...
      IF v_source_exists AND v_target_exists THEN
        ...
        -- Type-aware source expression.
        -- For text-like targets keep raw text; for others, empty string -> NULL then cast.
