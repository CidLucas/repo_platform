-- =============================================================================
-- Migration: Fallbacks for required analytics_v2.fato_transacoes columns
-- Date: 2026-04-22
--
-- Problem:
--   Dynamic mapped inserts can omit required columns in fato_transacoes
--   (tipo_id, data_competencia_id, valor), causing NOT NULL errors.
--
-- Fix:
--   1) Keep scientific-notation-safe integer casting
--   2) For NOT NULL mapped int/numeric/bool targets, apply COALESCE fallback
--   3) If destino is analytics_v2.fato_transacoes and required columns were
--      not mapped, append deterministic defaults:
--        - tipo_id = 0
--        - data_competencia_id = to_char(current_date, 'YYYYMMDD')::int
--        - valor = 0::numeric
-- =============================================================================
		...
		LOOP
			...
			IF v_source_exists AND v_target_exists THEN
				...
				IF v_target_nullable = 'NO' THEN
					IF v_target_udt_name IN ('int2', 'int4', 'int8') THEN
