-- =============================================================================
-- Migration: Handle BigQuery FDW epoch timestamps in extract_bigquery_data
-- Date: 2026-04-22
--
-- Problem:
--   BigQuery FDW emits TIMESTAMP columns as text containing UNIX epoch seconds
--   in scientific notation (e.g. '1.776873724E9'). Current casts:
--     - NULLIF(x,'')::timestamptz  → invalid input syntax
--     - NULLIF(x,'')::numeric::int4 via to_char → same failure because we cast
--       to timestamptz first.
--
-- Fix:
--   Create public.parse_bq_timestamp(text) → timestamptz that:
--     • returns NULL for empty
--     • if text matches numeric/scientific pattern → to_timestamp(numeric)
--     • otherwise → ::timestamptz
--   Use it in extract_bigquery_data for timestamp/date targets and for
--   YYYYMMDD date-key conversion.
-- =============================================================================

BEGIN;

CREATE OR REPLACE FUNCTION public.parse_bq_timestamp(p_text text)
RETURNS timestamptz
...
BEGIN
	IF v IS NULL THEN
		RETURN NULL;
	END IF;
	-- Numeric or scientific notation → treat as UNIX epoch seconds
...
GRANT EXECUTE ON FUNCTION public.parse_bq_timestamp(text) TO authenticated, service_role, postgres;
	...
	ELSE
		...
		LOOP
			...
			SELECT EXISTS (
				...
				WHERE table_schema = v_dest_schema AND table_name = v_dest_table AND column_name = v_val
			...
			IF v_source_exists AND v_target_exists THEN
				...
				WHERE c.table_schema = v_dest_schema AND c.table_name = v_dest_table AND c.column_name = v_val
				LIMIT 1;
				...
				IF v_is_date_key THEN
					-- Any BQ-style value → timestamptz → YYYYMMDD int
					v_expr := format(
						$$NULLIF(to_char(public.parse_bq_timestamp(%s::text), 'YYYYMMDD'), '')::%s$$,
						quote_ident(v_key),
						v_target_cast_type
					);
				...
				ELSIF v_target_udt_name IN ('int2', 'int4', 'int8') THEN
					v_expr := format('NULLIF(%s::text, '''')::numeric::%s', quote_ident(v_key), v_target_cast_type);
				ELSIF v_target_udt_name IN ('timestamp', 'timestamptz') THEN
					v_expr := format('public.parse_bq_timestamp(%s::text)::%s', quote_ident(v_key), v_target_cast_type);
				ELSIF v_target_udt_name = 'date' THEN
					v_expr := format('public.parse_bq_timestamp(%s::text)::date', quote_ident(v_key));
				ELSE
					v_expr := format('NULLIF(%s::text, '''')::%s', quote_ident(v_key), v_target_cast_type);
				END IF;
