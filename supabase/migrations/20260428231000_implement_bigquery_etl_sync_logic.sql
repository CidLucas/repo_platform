-- =============================================================================
-- Migration: Implement actual BigQuery→PostgreSQL ETL sync logic
-- Date: 2026-04-28
-- Purpose: Complete sincronizar_dados_cliente with real data transformation
-- =============================================================================
...
-- =============================================================================
-- RPC Function: sincronizar_dados_cliente (FULL IMPLEMENTATION)
-- Processes a sync job by:
-- 1. Querying the BigQuery foreign table
-- 2. Mapping columns using client_data_sources.column_mapping
-- 3. Upserting into dimension tables
-- 4. Inserting into fato_transacoes
-- =============================================================================
...
BEGIN
    ...
    BEGIN
        ...
        -- Build dynamic query to read from foreign table
        v_query := format('SELECT * FROM public.%I', v_ft_name);

        -- Process each row from the foreign table
        ...
        LOOP
            ...
            v_transacao_id := md5(v_client_id::TEXT || ':' || v_documento || ':' || v_data_competencia || ':' || COALESCE(v_produto_sku, ''));
            ...
            IF v_rows_affected % 100 = 0 THEN
                UPDATE analytics_v2.reg_jobs
                SET
                    progress_pct = LEAST(90, 10 + (v_rows_affected / 10)::INTEGER),
                    updated_at = now()
                WHERE job_id = p_job_id;
            END IF;
...
'Process a queued BigQuery sync job. Queries the foreign table, maps columns, and upserts into analytics_v2 fact/dimension tables.';
