...
-- CSV ETL fixes
--
-- 1. client_data_sources.schema_type: upload-csv-source persists the
--    schema classification chosen per file during onboarding (invoices /
--    fato_transacoes / dim_clientes / dim_inventory), but the column was
...
-- =============================================================================
-- 2. sincronizar_csv_cliente: accept 'transaction_label' mapping key
--
-- The Python schema matcher (tool_pool_api match_columns, INVOICES_COLUMNS)
-- emits the canonical column 'transaction_label' for source columns like
-- natureza_operacao / tipo_nf, but this function only looked up the mapping
-- key 'tipo_lancamento' -- so tier-1 tipo_transacao classification never
-- received the CSV label.
--
-- Body taken from the LIVE prod definition (2026-07-07), which is ahead of
-- the 20260523999999 baseline: it resets v_tipo_transacao/v_entry_type per
-- row and strips the time component in DD/MM/YYYY dates. On top of that:
-- transaction_label fallback + SET search_path (as the baseline intended).
--
-- 3. tipo_transacao tier-1: 'serviço'/'servico' removed from the compra
--    keywords. A service NF is a sale for the issuer and a purchase for
--    the contractor — the label alone cannot decide, so it falls through
--    to tiers 2/3 which look at the row structure (customer vs supplier).
--    Verified with novatech-ti: 131 recurring-service sales were being
--    classified as 'compra'.
-- =============================================================================
