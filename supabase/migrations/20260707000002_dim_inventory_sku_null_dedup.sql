-- dim_inventory: dedup de produtos sem SKU
--
-- Problema: o único índice de unicidade era (client_id, sku) WHERE sku IS NOT NULL.
-- Produtos identificados só por nome (sku NULL) nunca casavam no ON CONFLICT do
-- sincronizar_csv_cliente, então cada execução do ETL re-inseria todos os nomes
-- (ex.: 301 linhas para 26 nomes distintos em um cliente).
--
-- 1) Dedup das linhas existentes (mantém o menor inventory_id por (client_id, nome),
...
-- upsert do dim_inventory alterado. Não regenerar a partir do baseline.

BEGIN;

-- ── 1. Dedup das linhas sku-null existentes ─────────────────────────────────
WITH keepers AS (
  SELECT client_id, nome, MIN(inventory_id) AS keep_id
  FROM analytics_v2.dim_inventory
  WHERE sku IS NULL AND nome IS NOT NULL
  GROUP BY client_id, nome
),
dupes AS (
  SELECT di.inventory_id, k.keep_id
  FROM analytics_v2.dim_inventory di
  JOIN keepers k
    ON k.client_id = di.client_id AND k.nome = di.nome
  WHERE di.sku IS NULL
    AND di.inventory_id <> k.keep_id
)
UPDATE analytics_v2.fato_transacoes ft
SET produto_id = d.keep_id
FROM dupes d
WHERE ft.produto_id = d.inventory_id;

DELETE FROM analytics_v2.dim_inventory di
USING (
  SELECT client_id, nome, MIN(inventory_id) AS keep_id
  FROM analytics_v2.dim_inventory
  WHERE sku IS NULL AND nome IS NOT NULL
...
WHERE di.sku IS NULL
  AND di.client_id = k.client_id
  AND di.nome = k.nome
  AND di.inventory_id <> k.keep_id;

-- ── 2. Unicidade por (client_id, nome) quando sku é NULL ────────────────────
CREATE UNIQUE INDEX IF NOT EXISTS dim_inventory_client_nome_uniq
  ON analytics_v2.dim_inventory (client_id, nome)
  WHERE sku IS NULL;

-- ── 3. RPC: upsert por nome quando sku é NULL ───────────────────────────────
