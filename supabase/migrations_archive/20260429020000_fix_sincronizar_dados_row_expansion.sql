-- =============================================================================
-- Migration: Fix row expansion error in sincronizar_dados_cliente
-- Date: 2026-04-29
-- Problem: (v_ft_record).* ->> key is invalid PL/pgSQL — PostgreSQL does not
--   support row expansion via "*" in that expression context.
-- Fix: Convert each fetched record to JSONB with row_to_json() and use
--   v_row_json ->> computed_key for all dynamic field access.
-- =============================================================================
...
BEGIN
    ...
    SELECT j.job_id, j.client_id, j.credential_id, j.input_params, j.status
    INTO v_job
    ...
    SET status = 'running', started_at = now(), progress_pct = 10, updated_at = now()
    WHERE job_id = p_job_id;

    BEGIN
        SELECT ds.id, ds.column_mapping, bft.foreign_table_name
        INTO v_data_source_id, v_column_mapping, v_ft_name
        ...
        LOOP
            ...
            -- Convert the record to JSONB so fields can be addressed by computed key names
            v_row_json := row_to_json(v_ft_record)::jsonb;

            v_rows_affected := v_rows_affected + 1;

            v_documento        := v_row_json ->> (v_column_mapping->>'id_operatorinvoice');
            v_data_competencia := v_row_json ->> (v_column_mapping->>'createdat_operatorinvoice');
            v_quantidade       := (v_row_json ->> (v_column_mapping->>'quantitytraded_product'))::NUMERIC;
            v_valor_unitario   := (v_row_json ->> (v_column_mapping->>'unitprice_product'))::NUMERIC;
            v_valor            := (v_row_json ->> (v_column_mapping->>'totalprice_product'))::NUMERIC;
            v_status           := v_row_json ->> (v_column_mapping->>'status_operatorinvoice');

            v_cliente_cpf_cnpj := v_row_json ->> (v_column_mapping->>'receiverlegaldoc');
            v_cliente_nome     := v_row_json ->> (v_column_mapping->>'receiverlegalname');
            v_cliente_telefone := v_row_json ->> (v_column_mapping->>'receiverphone');
            v_cliente_cidade   := v_row_json ->> (v_column_mapping->>'receivercity');
            v_cliente_uf       := v_row_json ->> (v_column_mapping->>'receiverstateuf');

            v_fornecedor_cnpj     := v_row_json ->> (v_column_mapping->>'emitterlegaldoc');
            v_fornecedor_nome     := v_row_json ->> (v_column_mapping->>'emitterlegalname');
            v_fornecedor_telefone := v_row_json ->> (v_column_mapping->>'emitterphone');
            v_fornecedor_cidade   := v_row_json ->> (v_column_mapping->>'emittercity');
            v_fornecedor_uf       := v_row_json ->> (v_column_mapping->>'emitterstateuf');

            v_produto_sku  := v_row_json ->> (v_column_mapping->>'id_product');
            v_produto_nome := v_row_json ->> (v_column_mapping->>'description_product');

            v_transacao_id := md5(
                v_client_id::TEXT || ':' || COALESCE(v_documento, '') || ':' ||
                COALESCE(v_data_competencia, '') || ':' || COALESCE(v_produto_sku, '')
            );
            ...
            IF v_cliente_cpf_cnpj IS NOT NULL OR v_cliente_nome IS NOT NULL THEN
                INSERT INTO analytics_v2.dim_clientes (
                    ...
                    v_client_id, v_cliente_cpf_cnpj, v_cliente_nome, v_cliente_telefone,
                    v_cliente_cidade, v_cliente_uf, now()
                )
            ...
            IF v_fornecedor_cnpj IS NOT NULL OR v_fornecedor_nome IS NOT NULL THEN
                INSERT INTO analytics_v2.dim_fornecedores (
                    ...
                    v_client_id, v_fornecedor_cnpj, v_fornecedor_nome, v_fornecedor_telefone,
                    v_fornecedor_cidade, v_fornecedor_uf, now()
                )
            ...
            IF v_rows_affected % 100 = 0 THEN
                UPDATE analytics_v2.reg_jobs
                SET progress_pct = LEAST(90, 10 + (v_rows_affected / 10)::INTEGER), updated_at = now()
                WHERE job_id = p_job_id;
            END IF;
