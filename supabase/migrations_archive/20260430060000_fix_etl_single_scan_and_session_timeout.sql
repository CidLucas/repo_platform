-- Migration: fix statement_timeout override + replace O(n²) batch loop with single FDW scan
--
-- Root cause of timeout at batch 3:
--   The BigQuery Wrappers FDW creates internal subtransactions/savepoints.
--   When those subtransactions are released, PostgreSQL pops the SET LOCAL
--   stack, restoring statement_timeout from 30000ms (database-level ALTER DATABASE).
--   Combined with LIMIT/OFFSET being O(n²) on FDW foreign tables (batch at
--   OFFSET 30000 re-scans 45k rows from BigQuery), batch 3 always exceeds 30s.
--
-- Fix 1: SET LOCAL statement_timeout = 0 explicitly in the function body (runtime),
--         immune to subtransaction stack pops. Belt-and-suspenders with proconfig.
-- Fix 2: Single INSERT SELECT without LIMIT/OFFSET — one O(n) FDW scan.
--         BigQuery FDW supports up to 90 minutes per query (Wrappers default).
...
DECLARE
  ...
  v_duration            numeric;
BEGIN
  -- Belt-and-suspenders: override statement_timeout at session level too,
  -- immune to subtransaction stack resets caused by FDW internal savepoints.
  ...
  -- ── Invert column mapping: {source_col: canonical} → find key WHERE value = canonical ──
  ...
  -- ── Clear leftover staging rows from any previous attempt ─────────────────────
  ...
  -- ── Single FDW scan → staging (no LIMIT/OFFSET, O(n), up to 90min per Wrappers) ─
  -- statement_timeout = 0 ensures PostgreSQL never cancels this statement.
  EXECUTE format(
    'INSERT INTO analytics_v2.etl_staging
     ...
     SELECT %L, %s FROM %I',
    p_job_id, v_select, v_ft_name
  );
  GET DIAGNOSTICS v_staged = ROW_COUNT;

  RAISE NOTICE '[run_etl_job] job=%: staged % rows from %', p_job_id, v_staged, v_ft_name;
